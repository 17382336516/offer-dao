...
def headers(bvid: str | None = None) -> dict[str, str]:
    ...
    # 注入系统级 B站登录态：player/v2 在游客态下不返回字幕列表（subtitles 恒为空，
    # 需登录字幕归到 need_login_subtitle）。带登录 cookie 才能看到字幕，从而让
    # 字幕探查 / 提取在游客态之外正常工作。从环境变量 BILI_COOKIE 读取（与后端约定一致）。
    # 未配置时保持游客态，向后兼容。
    cookie = os.environ.get("BILI_COOKIE") or os.environ.get("BILI_SESSDATA")
    if cookie:
        h["Cookie"] = cookie
    return h
...
def check_subtitles(bvid: str, pages: list[dict]) -> list[dict]:
    # 注：player/v2 在带登录 cookie 时即可返回 AI 字幕轨道（subtitles 含 ai-zh）。
    # 实测加 WBI 签名反而会让 B站 风控返回空字幕列表，故这里用无签名请求 + 登录 cookie，
    # 由 headers() 注入 BILI_COOKIE 即可稳定拿到字幕（与 node 侧调用一致）。
    result = []
    for page in pages:
